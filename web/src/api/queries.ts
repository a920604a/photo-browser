...
function usePagedQuery<T>(api: ApiClient, key: readonly unknown[], path: string) {
...
export function useAlbumPhotos(albumId: number) {
  return usePagedQuery<Photo>(useApi(), ["photos", { albumId }], `/albums/${albumId}/photos`);
}

export function useAllPhotos() {
  return usePagedQuery<Photo>(useApi(), ["photos", "all"], "/photos");
}

export function useTimeline() {
  return usePagedQuery<Photo>(useApi(), ["photos", "timeline"], "/photos/timeline");
}
